	...
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			...
			<view v-else-if="n.name=='a'" :id="n.attrs.id" :class="'_a '+(n.attrs.class||'')" hover-class="_hover" :style="n.attrs.style"
			 :data-attrs="n.attrs" @tap.stop="linkpress" style="color: #F76D45;text-decoration:none">
			...
			<!--#endif-->
			<!--富文本-->
			<!--#ifdef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU || APP-PLUS-->
			<rich-text v-else-if="handler.use(n)" :id="n.attrs.id" :class="'_p __'+n.name" :nodes="[n]" />
			<!--#endif-->
			<!--#ifndef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU || APP-PLUS-->
