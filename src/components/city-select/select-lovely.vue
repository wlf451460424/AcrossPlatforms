<template>
	...
	<view class="city-select">
		<scroll-view :scroll-top="scrollTop" scroll-y="true" class="city-select-main" id="city-select-main" :scroll-into-view="toView">
			...
			<!-- 热门城市 -->
			<!-- <view class="hot-title" v-if="hotCity.length > 0 && !serachCity">猜你喜欢</view>
			<view class="hot-city" v-if="hotCity.length > 0 && !serachCity">
				<template v-for="(item, index) in hotCity">
					<view :key="index" @click="cityTrigger(item, 'hot')" class="hot-item">
						<view style="display: flex;width: 100%;justify-content: center;">
							<image :src="decodeURIComponent(item.url)" ></image>
						</view>
						<text>{{ item[formatName] }}</text>
					</view>
				</template>
			</view> -->
			<!-- 城市列表(搜索前) -->
			<!-- <view class="hot-title">全部</view> -->
			<view class="citys" v-if="!serachCity">
				<view v-for="(city, index) in sortItems" :key="index" v-show="city.isCity">
					...
					<view class="citys-item" v-for="(item, inx) in city.citys" :key="inx" @click="cityTrigger(item)">
						<view style="width: 100upx;height: 100%;padding-right: 40upx;">
							<image :src="decodeURIComponent(item.breedAvatar)" style="width: 60upx;height: 60upx;border-radius: 60upx;"></image>
						</view>
						<text style="width: 100%;height: 100%;">{{ item.cityName }}</text>
						<!-- <view style="width: 100%;height: 100%;display: flex;justify-content: flex-end;margin-right: 80upx;">
							<checkbox class="checkStyle"></checkbox>
						</view> -->
					</view>
				</view>
			</view>
			...
			<view v-if="!cityData.length" style="display: flex;width: 100%;justify-content: center;">
				<text style="line-height: 80upx;color: #b7b7b7;">暂无数据</text>
			</view>
		</scroll-view>
		<!-- 城市选择索引-->
		...
		buildCityindexs() {
			...
			for (let i = 0; i < this.cityData.length; i++) {
				...
				this.handleCity[index].citys.push({
					...
					breedAvatar:this.cityData[i].breedAvatar,
					unicode: unicode,
					oldData: this.cityData[i]
				})
			}
