				...
				<swiper-item>
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;">
							<!-- 小组 -->
							<view  v-for="(item,index) in list" :key="index" style="flex-direction:row;width: 100%;border-radius: 4upx;background-color: #FFFFFF;margin-bottom:2upx;padding:12upx;align-items: center;" >
								<view style="width:80%;align-items: center;">
									<image @click="gotoPersonalPage(item)" :src="decodeURIComponent(item.followTargetAvatar)"  style="width: 100upx;height: 100upx;border-radius: 60upx;"></image>
									<view style="flex-direction: column;width: 75%; margin-left: 20upx;">
										<view style="width: 100%;align-items: center;">
											<text style="max-width: 80%;line-height:50upx;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;">
												{{item.followTargetNickname}}
											</text>
											<text style="margin-left:12upx;font-size: 24upx;height: 30upx;line-height: 30upx;padding:0 4upx;background-color: #ffbf59;border-radius: 6upx;color: #FFFFFF;">
												{{item.growthValue}}
											</text>
										</view>
										<text style="width: 100%;line-height:50upx;font-size: 24upx;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;" >
											简介 : {{item.followTargetProfile?item.followTargetProfile:'此用户很懒，什么都没有留下'}}
										</text>
									</view>
								</view>
								<view  v-if="!item.isShowCareBtn && !item.isMyself" @click="ClickCareBtn(item)" style="width: 20%;align-items: flex-end;justify-content: flex-end;">
								...
								<view  v-if="item.isShowCareBtn && !item.isMyself" @click="ClickCareBtn(item)" style="width: 25%;align-items: flex-end;justify-content: flex-end;">
		...
		methods: {
			...
			getPostList(type){
				let param={
					...
					"followTargetType": this.type,//7:用户 8：话题 9：小组
				}
					...
					res => {
						if(res.data.code == 200 ){
							for(var i=0;i<this.list.length;i++){
								if(item.followUserId == this.list[i].followUserId){
