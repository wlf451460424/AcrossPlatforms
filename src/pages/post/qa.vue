<template>
	<view class="content">
		<view v-if="query_data.postAuditStatus!= 0 && query_data.isDeleted == 0 && query_data.postStatus != 2">
			...
			<!-- #endif -->	
				<view style="flex-direction: column; padding:12upx 24upx 0 24upx;background-color: #FFFFFF;">
					...
					<scroll-view scroll-x="true" style="white-space: nowrap;flex-direction: row;width: 100%;"  >
						<view v-if="query_data.topicTitle" @click="gotoTopicListPage(query_data.postTopicId)" style="margin-top: 12upx;padding:12upx 18upx;display: inline-flex;align-items: center;border-radius: 30upx;background-color: #f9f9f9;align-items: center;">
					...
					<view v-if="query_data.postContentType == 0 && query_data.postImage && (query_data.postType == 2 || query_data.postType == 3)" @click="gotoImageShow(query_data.postImage)" style="flex-direction: row;width: 100%;margin-top:12upx;flex-wrap: wrap;">
					...
					<scroll-view scroll-x="true" style="white-space: nowrap;flex-direction: row;width: 100%;color: #888888;margin-top:12upx;"  >
						<view v-for="(item,index) in postLabel" :key="index" @click="gotoTagListPage(item,query_data.postType)" style="line-height: 36upx;padding: 0 16upx 3upx;display: inline-block;margin-right: 20upx;border: 1px solid #f76d45;align-items: center;border-radius: 30upx;">
				...
				<view style="flex-direction: row;width: 100%;padding:24upx;border-top: 8upx solid #F9F9F9;border-bottom: 8upx solid #F9F9F9;">
					<view style="width:80%;">
						...
						<view style="flex-direction: column; margin-left: 20upx;width: 80%;align-items: center;">
				...
				<view style="flex-direction: row;width: 100%;margin-top: 8upx;align-items: center;padding:0 24upx; background-color: #FFFFFF;"  >
					<view style="width: 100%;height: 80upx;align-items: center;">
						<view @click="changeItem" style="margin-right:60upx;" :class="{item_active: !commentORupvote}" >
							<text style="font-weight: bold;margin-right: 10upx;">{{commentOrqa}}</text>
				...
				<view  v-if="!commentORupvote" style="flex-direction: column;width: 100%;">
					<view v-for="(item,index) in commentList" :key="index" style="flex-direction: column;width: 100%;padding:12upx 24upx;border-bottom: 2upx solid #F9F9F9;">
						...
						<view @click="gotoHuifu_page(item)" v-if="item.replyCount>0?true:false" style="width: 100%;height: 50upx;padding: 0 20upx;">
					...
					<view v-if="commentTotal>5" @click="gotoPinlun_page" style="width: 100%;height:80upx;align-items:center;justify-content: center;color: #f76d45;">
						<!-- #ifdef H5  -->
						<text style="color: #f76d45;">打开APP查看更多回答</text>	
						<!-- #endif -->
						<!-- #ifdef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU  -->
						<text style="color: #f76d45;">查看更多回答</text>	
						<!-- #endif -->
					</view>
					<view v-if="commentTotal==0" style="width: 100%;height:80upx;align-items:center;justify-content: center;color: #f76d45;">
						<text style="color: #b7b7b7;">快去发表你的评论吧</text>	
					</view>
				</view>
			</view>
		...
		methods: {
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200){
							...
							//问答中闲置更换文字
							if(this.query_data.postTagId.split(',').indexOf('1278492374447689730') > -1){
								this.isIdle = 1;
							}else{
								this.isIdle = 0;
							}
			...
			getCommentList(){
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200){
							this.commentTotal = this.calculation.digitalConversion(this.commentTotal)
