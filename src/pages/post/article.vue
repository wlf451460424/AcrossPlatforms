<template>
	<view class="content">
		<view v-if="query_data.isDeleted == 0  && query_data.postStatus != 2">
			<view style="flex-direction: column;width: 100%; margin-bottom: 100upx;">
				<swiper v-if="query_data.postType == 1 && query_data.postImage" @click="gotoImageShow(query_data.postImage)" class="swiper-style" indicator-dots="true" :autoplay="true" :interval="2000" :duration="500">
				...
				<view style="flex-direction: column; padding:24upx 24upx 0 24upx;background-color: #FFFFFF;">
					...
					<view style="flex-direction: column;">
						...
						<text :class="{active:allContent}" ref="showAllText" @click="showAllText" style="width: 100%;line-height: 60upx;text-align:center;color: #f76d45;">
							点击阅读全文
						</text>
					...
					<view v-if="query_data.postImage && (query_data.postType == 2 || query_data.postType == 3)" @click="gotoImageShow(query_data.postImage)" style="flex-direction: row;width: 100%;margin-top:12upx;flex-wrap: wrap;">
				...
				<view style="flex-direction: row;width: 100%; height: 100upx; line-height: 100upx;background-color: #FFFFFF;padding: 0 24upx;margin-top: 2upx;
					align-items: center;">
					<view style="min-width: 5%;max-width: 20%;flex-direction: column;justify-content: center;margin-right: 20upx;font-size: 24upx;color: #b7b7b7;" @click="clcik_thumbsup">
					...
					<scroll-view  scroll-x="true" style="min-width: 75%;max-width: 90%;white-space: nowrap;flex-direction: row;height: 60upx;line-height: 70upx;align-items: center; color: #888888;"  >
						...
						<image v-if="upvoteList.length==8" @click="thumbsup_list" src="@/static/actionImg/more_bg.png" style="width: 60upx; height: 60upx;display: inline-block;"></image>
					</scroll-view>
				...
				<view v-if="recommendList.length" style="width: 100%;height:70upx;color: #333333;font-size: 14px;background-color: #FFFFFF;padding: 0 20upx;margin-top: 8upx;">
					<view style="width: 50%;height: 100%;align-items:center;font-weight: bold;">
						<text>相关推荐</text>
					</view>
					<view @click="more_postView" style="font-size: 24upx;width: 50%;height: 100%;align-items:center;justify-content: flex-end;color: #c7c7c7;">
						<text>更多</text>
						<image  src="@/static/mine-ico/next.png" style="margin-left: 20upx;width: 16upx; height: 26upx;"></image>
					</view>
				</view>
				<!-- 相关推荐 -->
				<view v-if="recommendList.length" style="flex-direction: column;width: 100%;padding: 2upx 0upx;">
					<!-- 文章 -->
					<articleItem v-for="(item,index) in recommendList" :key="index" :receive-obj="{obj:item}"></articleItem>
					
					<!-- 文章 recommendList-->
					<!-- <view v-for="(item,index) in recommendList" :key="index" style="flex-direction:column;width: 100%;padding:12upx; border-radius: 4upx; background-color: #FFFFFF;margin-bottom:8upx;" >
						<view style="flex-direction: row;">
							<view style="width:85%;">
								<image :src="decodeURIComponent(item.postUserAvatar)" mode="aspectFill" style="width: 76upx;height: 76upx;border-radius: 50upx;"></image>
								<view style="flex-direction: column; margin-left: 20upx;width: 80%;">
									...
									<view style="width: 100%;align-items: center;color: #b7b7b7;font-size: 24upx;">
										<text>{{item.postFriendlyDate}}</text>
					...
					<view style="width: 100%;height:60upx;align-items:center;justify-content: center;">
						<view style="width: 35%;height: 10upx;border-bottom: 1px solid #f1f1f1;">
						</view>
						<view style="height: 100%;align-items:center;justify-content: center;padding: 0 10upx;">
							<text style="color: #b7b7b7;">我是有底线的</text>
						</view>
						<view style="width: 35%;height: 10upx;border-bottom: 1px solid #f1f1f1;">
						</view>
					</view>
		...
		methods: {
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200){
							...
							this.query_data.postContent = this.query_data.postContent.replace(/<img style="/g,'<img style="width:100%;')
							...
							//提取图片
							var str = this.query_data.postContent
							...
							this.query_data.postImageShow = arr[0].match(srcReg)[1];
						}else{
							uni.showToast({
							    title: res.data.msg,
								icon: 'none',
			...
			getRecommendList(){
				let param={
					"limit": 4,
					"page": 1,
					"postTagId": this.query_data.postTagId,
					"postType": this.query_data.postType,//"0: 文章 1: 笔记 2: 动态 3: 问答 4: 新闻 5: 评论 6: 回复",
					"postId":this.recive_data.postId
				} ;
					...
					res => {
						if(res.data.code == 200){
							...
							for(let i=0;i<this.recommendList.length;i++){
								// 处理赞 评论 浏览数量
								this.recommendList[i].postUpvoteCount = this.calculation.digitalConversion(this.recommendList[i].postUpvoteCount)
								this.recommendList[i].postCommentCount = this.calculation.digitalConversion(this.recommendList[i].postCommentCount)
								this.recommendList[i].postPageViews = this.calculation.digitalConversion(this.recommendList[i].postPageViews)
								
								this.recommendList[i].postFriendlyDate = friendlyDate(new Date(this.recommendList[i].updateTime.replace(/\-/g,'/')).getTime(),this.recommendList[i].updateTime)
								this.recommendList[i].postImageShow = this.recommendList[i].postImage.split(",")[0];
							}
							if(this.recive_data.postType == 0){
								for(let i=0;i<this.recommendList.length;i++){
									var str = this.recommendList[i].postContent
									//匹配图片（g表示匹配所有结果i表示区分大小写）
									var imgReg = /<img.*?(?:>|\/>)/gi;
									//匹配src属性
									var srcReg = /src=[\'\"]?([^\'\"]*)[\'\"]?/i;
									...
									this.recommendList[i].postImageShow = arr[0].match(srcReg)[1];
								}
							}
						}
					}
			...
			goActionItem_Details(item) {
				uni.navigateTo({
					url:'/pages/post/article?id='+item.postId+'&type='+item.postType
				})
			},
			gotoImageShow(imageString){
				uni.navigateTo({
					url:"/pages/pages/discover/post-details/post-image?imgArr=" + encodeURIComponent(imageString) 
				})
			},
			more_postView(){
				uni.navigateTo({
