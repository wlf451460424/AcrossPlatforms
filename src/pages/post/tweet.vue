<template>
	<view class="content">
		<view v-if="query_data.postAuditStatus!= 0 && query_data.isDeleted == 0 && query_data.postStatus != 2">
			<!-- #ifdef  H5 || MP-WEIXIN || MP-TOUTIAO || MP-QQ  -->
			<view style="flex-direction: column;width: 100%; margin-bottom: 100upx;">
			<!-- #endif -->
			<!-- #ifdef MP-BAIDU  -->
			<view style="flex-direction: column;width: 100%; margin-bottom: 100upx;min-height: 100vh;">
			<!-- #endif -->	
				<view style="flex-direction: column; padding:24upx 24upx 0 24upx;background-color: #FFFFFF;">
					...
					<scroll-view scroll-x="true" style="white-space: nowrap;flex-direction: row;width: 100%;"  >
						<view v-if="query_data.topicTitle" @click="gotoTopicListPage(query_data.postTopicId)" style="margin-top: 12upx;padding:12upx;display: inline-flex;align-items: center;border-radius: 10upx;background-color: #f9f9f9;align-items: center;">
					...
					<view style="flex-direction: column;">
						<!-- <text style="font-size: 32upx;font-weight: 700;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;overflow: hidden;">
							{{query_data.postTitle}}
						</text> -->
						<view style="width: 100%;overflow: hidden;flex-direction: column;">
							{{query_data.postTitle}}
						</view>
						<!-- #ifdef H5 -->
					...
					<view v-if="query_data.postContentType == 0 && query_data.postImage && (query_data.postType == 2 || query_data.postType == 3) && query_data.postPublisher!=1" @click="gotoImageShow(query_data.postImage)" style="flex-direction: row;width: 100%;margin-top:12upx;flex-wrap: wrap;">
						...
						<!-- <image v-if="query_data.postImage.split(',')[0]" :src="decodeURIComponent(query_data.postImage.split(',')[0])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[1]" :src="decodeURIComponent(query_data.postImage.split(',')[1])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[2]" :src="decodeURIComponent(query_data.postImage.split(',')[2])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[3]" :src="decodeURIComponent(query_data.postImage.split(',')[3])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[4]" :src="decodeURIComponent(query_data.postImage.split(',')[4])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[5]" :src="decodeURIComponent(query_data.postImage.split(',')[5])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[6]" :src="decodeURIComponent(query_data.postImage.split(',')[6])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[7]" :src="decodeURIComponent(query_data.postImage.split(',')[7])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image>
						<image v-if="query_data.postImage.split(',')[8]" :src="decodeURIComponent(query_data.postImage.split(',')[8])" mode="aspectFill"  style="width: 33.33%; height: 200upx; padding: 5upx;"></image> -->
					</view>
					<view v-if="query_data.postContentType == 0 && query_data.postImage && (query_data.postType == 2 || query_data.postType == 3) && query_data.postPublisher==1" @click="gotoImageShow(query_data.postImage)" style="flex-direction: row;width: 100%;margin-top:12upx;flex-wrap: wrap;">
						<image v-if="query_data.postImage.split(',')[0]" :src="decodeURIComponent(query_data.postImage.split(',')[0])" mode="aspectFill"  style="width:100%; height: 240upx; padding: 5upx;"></image>
					</view>
					
					<view v-if="query_data.postContentType === 1 && query_data.postAuditStatus === 1" style="position: relative;margin-top: 12upx;height: 396upx;">
						<!-- #ifdef H5 -->
					...
					<!-- <scroll-view scroll-x="true" style="white-space: nowrap;flex-direction: row;width: 100%;color: #888888;margin-top:12upx;"  >
						<view v-for="(item,index) in this.postLabel" :key="index" @click="gotoTagListPage(item,query_data.postType)" style="height:35upx;line-height:35upx;font-size: 24upx;padding: 0 8upx;display: inline-block;margin-right: 20upx;background-color:#f76d45;align-items: center;border-radius: 4upx;">
							<text style="color: #FFFFFF;font-size: 24upx;">{{item.tagName}}</text>		
						</view>
					</scroll-view> -->
					<view style="flex-direction: row;width: 100%;color: #b7b7b7;align-items: center;margin:12upx 0;"  >
						<view style="flex-direction: row;width: 80%;align-items: center;font-size: 24upx;">
							...
							<text style="width: 90%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;overflow: hidden;">
								{{query_data.postPoi}}
							</text>		
						</view>
					...
					<view v-if="false" style="flex-direction: row;width: 100%; height: 100upx; line-height: 100upx;
						align-items: center;border-top: 1px solid #b7b7b7;">
						<view style="flex-direction: column;justify-content: center;margin-right: 40upx;" @click="clcik_thumbsup">
						...
						<scroll-view scroll-x="true" style="white-space: nowrap;flex-direction: row;height: 60upx;line-height: 70upx;align-items: center; color: #888888;"  >
							<view v-for="(item,index) in upvoteList" :key="index" style="white-space: nowrap;margin-right: 10upx;display: inline-block;height: 60upx;">
							...
							<image v-if="upvoteList.length" src="@/static/actionImg/more_bg.png" style="width: 60upx; height: 60upx;display: inline-block;"></image>
						</scroll-view>
						<view >
							<image  @click="thumbsup_list" src="@/static/mine-ico/next.png" style="width: 16upx; height: 26upx;"></image>
						</view>
					</view>
				</view>
				
				<view style="flex-direction: row;width: 100%;align-items: center;padding:0 24upx; border-top: 8upx solid #F9F9F9;"  >
					<view style="width: 100%;height: 80upx;align-items: center;">
						<view @click="changeItem" style="margin-right:60upx;" :class="{item_active: !commentORupvote}" >
							<text style="font-weight: bold;margin-right: 10upx;">评论 </text>
							<text style="font-weight: bold;">{{commentTotal}}</text>
						</view>
						<view @click="changeItem" style="margin-right:30upx;" :class="{item_active: commentORupvote}">
							<text style="font-weight: bold;margin-right: 10upx;">点赞 </text>
							<text style="font-weight: bold;">{{upvoteTotal}}</text>
						</view>
					</view>
				</view>
				<view v-if="commentORupvote" style="flex-direction: column;width: 100%;">
					<view v-for="(item,index) in upvoteList" :key="index" style="width: 100%;padding:24upx;border-bottom: 2upx solid #F9F9F9;  " >
						<view style="flex-direction: row;width: 70%;align-items:center;height: 80upx;">
							<image @click="click_user(item.upvoteUserId)" :src="decodeURIComponent(item.upvoteUserAvatar)" mode="aspectFill" style="width: 80upx;height: 80upx;border-radius: 50upx;"></image>
							<view style="flex-direction: column; margin-left: 20upx;">
								<view style="width: 100%;align-items: center;">
									<text style="max-width: 85%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;">
										{{item.upvoteUserNickname}}
									</text>
									<text style="font-size: 24upx;height: 30upx;line-height: 30upx;margin-left: 12upx;padding:0 4upx;background-color: #ffbf59;border-radius: 4upx;color: #FFFFFF;">
										{{item.growthValue}}
									</text>
								</view>
								<text style="line-height: 40upx;color: #b7b7b7;font-size: 24upx;">{{item.upvoteFriendlyDate}}</text>
							</view>
						</view>
						<view v-if="!item.currentUserFollowStatus && item.upvoteUserId!=myUserId" @click="thumbsup(item)" style="width: 30%;justify-content: flex-end;align-items: center;">
							<view style="width: 100upx;height:50upx;background-color: #ffbf59;border-radius: 30upx;align-items: center;justify-content: center;">
								<text style="line-height: 50upx;font-size: 24upx;color: #FFFFFF;">关注</text>
							</view>
						</view>
					...
					<view v-if="upvoteTotal>5"  style="width: 100%;height:80upx;align-items:center;justify-content: center;color: #f76d45;">
						<!-- #ifdef H5  -->
						<text style="color: #f76d45;" @click="gotoPinlun_page">打开APP查看更多点赞</text>	
						<!-- #endif -->
						<!-- #ifdef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU  -->
						<text style="color: #f76d45;" @click="thumbsup_list">查看更多点赞</text>	
						<!-- #endif -->
					</view>
					<view v-if="upvoteTotal==0" style="width: 100%;height:80upx;align-items:center;justify-content: center;color: #f76d45;">
						<text style="color: #b7b7b7;">快去点赞吧</text>	
					</view>
				</view>	
				<view  v-if="!commentORupvote" style="flex-direction: column;width: 100%;">
					<view v-for="(item,index) in commentList" :key="index" style="flex-direction: column;width: 100%;padding:12upx 24upx;border-bottom: 2upx solid #F9F9F9; ">
			...
			<view class="foot" :class="{chat_style_1:commentORupvote}">
			...
			<view class="foot_mask" :class="{chat_style_1:commentORupvote}" v-if="!query_data.postStatus && !commentORupvote" @click="commentMask">
			</view>
			
			<view class="foot_right" :class="{chat_style_2:commentORupvote}">
...
<script>
	...
	export default {
		...
		methods: {
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200){
							temp.query_data = res.data.data;
							temp.postImageArr = res.data.data.postImage.toString().split(',')
							for( var j=0;j<temp.postImageArr.length;j++){
								if(temp.postImageArr[j]){
									temp.postImageArr[j] = decodeURIComponent(temp.postImageArr[j]).split("?")[0]
									if(temp.postImageArr[j].indexOf(".gif") != -1){
										temp.postImageGifArr.push("1")
									}else{
										temp.postImageGifArr.push("0")
									}
								}
							}
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200){
							...
							this.upvoteList = this.upvoteList.splice(0,5)

							for(var i=0;i<this.upvoteList.length;i++){
								this.upvoteList[i].growthValue = this.calculation.groupArr(this.upvoteList[i].growthValue).split(" ")[0]
								this.upvoteList[i].upvoteFriendlyDate = friendlyDate(new Date(this.upvoteList[i].createTime.replace(/\-/g,'/')).getTime(),this.upvoteList[i].createTime)
							}
			...
			// 	uni.navigateTo({
			// 		url:'post?id='+item.postId+'&type='+item.postType
			// 	})
			// },
			...
			// more_postView(){
			// 	uni.navigateTo({
			// 		url:'discover/post-details/post-recommend-more?postType=' + this.query_data.postType + '&postTagId' + this.query_data.postTagId
			// 	})
			// },
			...
			changeItem(){
				this.commentORupvote = !this.commentORupvote
				//根据帖子id查询评论列表
				this.getCommentList()
				//查询点赞列表
				this.getUpvoteList()
			...
			thumbsup(item){
				...
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200 ){
							for(var i=0;i<this.upvoteList.length;i++){
								if(item.upvoteUserId == this.upvoteList[i].upvoteUserId){
									this.upvoteList[i].currentUserFollowStatus = !this.upvoteList[i].currentUserFollowStatus
								}
							}
						}else{
							uni.showToast({
