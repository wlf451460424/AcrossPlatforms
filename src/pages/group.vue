<template>
	<view class="content">
		<view class="banner" style="z-index:2;flex-direction: column;">
		    <image class="banner-img" :src="decodeURIComponent(introduceData.groupLogo)"  mode="scaleToFill"></image>
			<view style="position: absolute;width:100%;height: 260upx;background: rgba(0,0,0,0.3);"></view>
			<image class="banner-img1" style="width: 80upx;height: 80upx;border-radius: 50upx;" mode="aspectFill" :src="decodeURIComponent(introduceData.groupAvatar)"></image>
			...
			<view class="banner-title1">
				<text style="font-size: 24upx;width: 100%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 3; overflow: hidden;" >
			...
			<view class="banner-title2">
				...
				<view style="width: 20%;justify-content: flex-end;color: #FFFFFF;font-size: 24upx;">
					<text v-if="introduceData.currentUserFollowStatus==0" @click="ClickCareBtn(introduceData)"  style="width: 120upx;height: 50upx;line-height: 50upx;text-align: center;padding: 0 10upx;border-radius: 40upx;background-color: #ffbf59;color: #FFFFFF;">
						加入
					</text>
					<text v-if="introduceData.currentUserFollowStatus==1" @click="ClickCareBtn(introduceData)" style="width: 120upx;height: 50upx;line-height: 50upx;text-align: center;padding: 0 10upx;border-radius: 40upx;border: 2upx solid #ffbf59;color: #ffbf59;">
						已加入
					</text>
				</view>
			</view>
		...
		<view v-if="introduceData.groupAnnouncementTitle" style="z-index:2;width: 100%;height: 60upx;background: #FFFFFF;padding: 0 20upx;margin-bottom: 2upx;align-items: center;">
			...
			<view class="uni-swiper-msg" style="padding:0;">
				<swiper class="swiper" vertical="true" autoplay="false" duration="300" interval="3000">
					<swiper-item>
						<view @click="noticToDetails(introduceData.groupAnnouncementLink)" style="line-height: 60upx;">{{introduceData.groupAnnouncementTitle}}</view>
					</swiper-item>
				</swiper>
			</view>
		</view>	
		...
		<!-- 下拉刷新组件 -->
		<mix-pulldown-refresh ref="mixPulldownRefresh" class="panel-content" :top="350" @refresh="onPulldownReresh">
		...
		<view v-if="menuView && introduceData.groupAnnouncementTitle"  style="z-index: 6;position: fixed;margin-top: 404upx;width: 100%;height:100%;background-color:rgba(255,255,255,0);flex-direction: column;">
		...
		<view v-if="menuView && !introduceData.groupAnnouncementTitle"  style="z-index: 6;position: fixed;margin-top: 344upx;width: 100%;height:100%;background-color:rgba(255,255,255,0);flex-direction: column;">
			<scroll-view scroll-y="true" style="max-height: 400upx;">
		...
		<uni-popup ref="showJion" type="bottom" style="z-index: 999;">
			<view style="flex-direction: column;width: 100%;background-color: #FFFFFF;">
				...
				<view @click="quit(introduceData)" style="width: 100%;height: 90upx;justify-content: center;align-items: center;">
					<text style="color: #FF0000;">退出小组</text>
				</view>
			</view>
		</uni-popup>
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
		    return {
		      title: this.introduceData.groupName,
		...
		onShareTimeline() {
			return {
				title: this.introduceData.groupName,
				imageUrl: this.introduceData.groupLogo,
			}
		},
		// #endif
		...
		methods: {
			...
			getGroupNameById(){
				...
				let str = '/portal/v1/group/get/' + this.GroupId
			...
			//获取标签
			getLabel(){
				let param={"classificationTagType":0};//分类标签所属类型 0：小组/1: 话题
			...
			getPostList(type){
				...
				if(this.tabCurrentIndex == 0){
					param={
						...
						"postGroupId": this.GroupId,//根据话题查询帖子
						"sortType":0,//全部
					}
			...
			ClickCareBtn(item){
				this.menuView = false
				if(item.currentUserFollowStatus){
			...
			look(item){
				...
				uni.navigateTo({
					url:'/pages/group/group-member?id=' + item.groupId +'&type=9'
				})
			},
			//退出小组
			quit(item){
			...
			goMemberList(value) {
				uni.navigateTo({
					url:'/pages/group/group-member?groupId=' + value
				})
			},
