					...
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;">
							<view v-for="(item,index) in list" :key="index" style="width: 100%;height: 120upx;align-items: center;background-color: #FFFFFF;padding: 0 12upx;margin-top: 2upx;">
								...
								<view style="width: 87%;flex-direction: column;" @click="go_Details(item.messageId,item.messageRecordTargetId,item.messageRecordTargetType)">
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			let param={
				// "message_type":1,   //消息类型 0：赞/1：收藏/2：评论/3：回复/4：关注
				"message_type": 11, //关注目标类型 //关注目标类型 0-文章 1-笔记 2-动态 3-问答 4-新闻 5-评论 6-回复 7-用户 8-话题 9-小组 10-赞 11-收藏 12-分享 13-通知  14-关注
			} ;
		...
		methods: {
			...
			getList(type){
				let param={
					"limit": this.limit,
					"page": this.page, 
					// "messageType": 1,//消息类型 0：赞/1：收藏/2：评论/3：回复/4：关注
					"messageType": 11, //关注目标类型 //关注目标类型 0-文章 1-笔记 2-动态 3-问答 4-新闻 5-评论 6-回复 7-用户 8-话题 9-小组 10-赞 11-收藏 12-分享 13-通知  14-关注
				} ;
			...
			go_Details(messageId,PostId,PostType) {
				let param={"messageId":messageId} ;
				let str = '/portal/v1/message/update/' + messageId;
				...
				for(let i=0;i<this.list.length;i++){
					if(messageId == this.list[i].messageId){
						this.list[i].messageStatus = 1
					}
				}
				...
				if(PostType == 0 ){//文章
					uni.navigateTo({
						url:this.navigatorUrl.articleDetailsPages + '?id=' + PostId
					})
				}
				if(PostType == 1){//笔记
					uni.navigateTo({
						url:this.navigatorUrl.noteDetailsPages + '?id=' + PostId
					})
				}
				if(PostType == 2){//动态
					uni.navigateTo({
						url:this.navigatorUrl.tweetDetailsPages + '?id=' + PostId
					})
				}
				if(PostType == 3){//问答
					uni.navigateTo({
						url:this.navigatorUrl.qaDetailsPages + '?id=' + PostId
					})
				}
			},
			//更多操作
