	...
	<view class="content">
		<scroll-view scroll-y="true">
			...
			<view style="width: 100%;height: 160upx;align-items: center;" v-if="item.remindPlanStatus==0">
				...
				<view style="width: 73%;height: 120upx;line-height: 60upx;flex-direction: column;border-radius: 6upx;background-color: #63d295;padding: 20upx;justify-content: center;">
					<view style="flex-direction: row;color: #FFFFFF;">
						<view style="width: 80%;">
					...
					<view v-if="item.remindPlanMedicine" style="flex-direction: row;color: #FFFFFF;line-height: 60upx;height: 60upx;">
						<text style="width: 60px;">药品名称:</text>
						<text style="display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;overflow: hidden;">
							{{item.remindPlanMedicine}}
						</text>
					</view>
				</view>
			</view>
			...
			<view style="width: 100%;height: 160upx;align-items: center;" v-if="item.remindPlanStatus==3">
				...
				<view style="width: 73%;height: 120upx;flex-direction: column;border-radius: 6upx;background-color: #F9F9F9;padding: 20upx;color: #121212;justify-content: center;">
					...
					<view  v-if="item.remindPlanMedicine" style="flex-direction: row;line-height: 60upx;height: 60upx;">
						<text style="width: 60px;">药品名称:</text>
						<text style="display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;overflow: hidden;">
							{{item.remindPlanMedicine}}
						</text>
...
<script>
	export default{
		data (){
			return{
				petAvatar:'../../../static/header_default.png',
				petAvatar_default:"../../../static/header_default.png",
				petName:'',
				list:[],
				titleName:'',
				listNow:[],
		...
		async onLoad(e) {
			let arr = e.ids.split("_")
			// 根据宠物ID查询宠物信息
			this.getPetDetails(arr[0])
			
			// console.log(e.ids)
			this.titleName = arr[2]
			uni.setNavigationBarTitle({
				title: this.titleName+ "记录"
			})
			//获取计划记录列表  参数：宠物id  类型id
			let param={remindPlanPetId:arr[0],remindPlanTypeId:arr[1]}
