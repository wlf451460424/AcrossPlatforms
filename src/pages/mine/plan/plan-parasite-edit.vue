<template>
	<view class="content">
		<view style="flex-direction: column;width: 100%;">
			<view style="flex-direction: row;width: 100%;height: 90upx;line-height: 90upx;margin-top: 2upx;background-color: #FFFFFF;padding: 20upx;">
				...
				<view style="width: 80%;height: 100%;align-items: center;">
					<input @input="uniPopupInput_onKeyInput" style="width: 100%;font-size: 12px;color: #5c5f66;" :value="remindPlanMedicine" maxlength="20" placeholder="请填写药名" />
				</view>
			</view>
			<view style="flex-direction: row;width: 100%;height: 90upx;line-height: 90upx;margin-top: 2upx;background-color: #FFFFFF;padding: 20upx;">
				<view style="width: 20%;height: 100%;align-items: center;">
		...
		<uniPopup ref="save_uniPopup" :mask-click="false" type="center">
			<view class="uni-tip">
				<!-- <text class="uni-tip-title" >个人简介</text> -->
	...
	export default {
					...
					res => {
						if(res.data.code == 200){
							...
							if(res.data.data.remindPlans.length){
								...
								this.remindPlanMedicine = res.data.data.remindPlans[0].remindPlanMedicine
		...
		methods: {
					...
					case 'del':
						...
						this.http.httpTokenRequest(opts,param).then(
							res => {
								if(res.data.code == 200){
									// console.log(res.data);
									let obj = this.planData.petId + "_" + this.planData.remindTypeId + "_" + this.planData.remindTypeName
									uni.navigateTo({
										url:"plan-parasite-timeline?ids=" + obj
									})
								}else{
									uni.showToast({
										title: res.data.msg,
			...
			save(){
				...
				if(this.add_OR_modify){//修改
					...
					let obj={
						...
						remindPlanMedicine: this.remindPlanMedicine,//药品名称
			...
			uniPopupInput_onKeyInput: function(event) {
				this.remindPlanMedicine = event.target.value
			},
			gotoHistoryRecords() {
