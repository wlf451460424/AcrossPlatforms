				...
				<swiper-item>
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;;">
							<view v-for="(item,index) in list" :key="index" @click="go_Details(item)" style="width: 100%;height: 120upx;align-items: center;background: #FFFFFF;padding: 0 12upx;margin-top: 2upx;">
								<view style="width: 13%;">
								...
								<view style="width: 87%;flex-direction: column;">
									<view style="width: 100%;">
										<view style="width: 70%;">	
											<text>赞了 </text>
											<text style="color: #f76d45;max-width: 200upx;overflow: hidden;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;">{{item.upvoteTargetUserNickname}}</text>
											的
											<text >{{typeText[item.upvoteTargetType]}}</text>
										</view>
									...
									<text style="color: #AAAAAA;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;" >
										{{item.upvoteTargetTitleOrContent}}
									</text>
								</view>
							</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			getPostList(type){
				...
				let opts={ url:'/portal/v1/upvote/target/pages', method:'POST' }
			...
			go_Details(item) {
				switch (item.upvoteTargetType) {
					case 0:
					case 1:
					case 2:
					case 3:
						if(item.upvoteTargetType == 0 ){//文章
							uni.navigateTo({
								url:this.navigatorUrl.articleDetailsPages + '?id=' + item.upvoteTargetId
							})
						}
						if(item.upvoteTargetType == 1){//笔记
							uni.navigateTo({
								url:this.navigatorUrl.noteDetailsPages + '?id=' + item.upvoteTargetId
							})
						}
						if(item.upvoteTargetType == 2){//动态
							uni.navigateTo({
								url:this.navigatorUrl.tweetDetailsPages + '?id=' + item.upvoteTargetId
							})
						}
						if(item.upvoteTargetType == 3){//问答
							uni.navigateTo({
								url:this.navigatorUrl.qaDetailsPages + '?id=' + item.upvoteTargetId
							})
						}
						break;
					case 5:
						uni.navigateTo({
							url:'comment-list-comment-details?id=' + item.upvoteTargetId
						})
						break;
					case 6:	
						uni.navigateTo({
							url:'comment-list-reply-details?id=' + item.upvoteTargetId + '&type=' + item.upvoteTargetType  + '&userId=' + item.upvoteUserId
						})
						break;
				}
			}
