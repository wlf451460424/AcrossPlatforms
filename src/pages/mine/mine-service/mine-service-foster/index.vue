...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap(e) {
			uni.navigateTo({
				url:"service-foster-placeList?id=" + uni.getStorageSync('userInfo').userId
			})
		},
		async onLoad(e) {
		...
		methods: {
			...
			getorderList(type){
				...
				let opts={ url:'/portal/v1/fosterage_order/list/status', method:'POST' }
			...
			go_Details(item) {
				uni.navigateTo({
					url:'service-foster-record-details?id=' + item.fosterageOrderId
				})
			},
