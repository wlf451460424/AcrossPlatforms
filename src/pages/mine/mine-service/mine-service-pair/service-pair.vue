				...
				<swiper-item>
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;flex-flow: wrap;padding:6upx 0 12upx 6upx;">
							<view v-for="(item,key) in list" :key="key" @touchend.prevent="tap(item)"
								...
								<view style="flex-direction: column;">
									<image :src="decodeURIComponent(item.pairPetImage)" mode="aspectFill" style="width: 100%; height: 410upx; border-radius: 4upx; "></image>
									<view style="flex-direction: column;padding-top: 6upx;">
										<view style="width: 100%;padding:0 12upx;">
											...
											<text style="width: 100upx;">{{item.score}}分</text>
										</view>
								...
								<view v-if="item.pairAuditResult == 0" style="position: relative;top:-530upx;left:0;width: 100%;height: 530upx;background: #FFFFFF;opacity: 0.6;border-radius: 4upx;align-items: center;justify-content: center;padding-bottom: 130upx;">
								...
								<view v-if="item.pairAuditResult == 1" style="position: relative;top:-530upx;left:0;width: 100%;height: 530upx;background: #FFFFFF;opacity: 0.6;border-radius: 4upx;align-items: center;justify-content: center;padding-bottom: 130upx;">
									<view style="width: 140upx;height: 140upx;border-radius: 100upx; background-color: #000000;align-items: center;justify-content: center;">
										<text style="color: #FFFFFF;font-size: 28upx;">审核失败</text>
									</view>
								</view>
								<view v-if="item.pairAuditResult == 2 && item.pairStatus == 1" style="position: relative;top:-530upx;left:0;width: 100%;height: 530upx;background: #FFFFFF;opacity: 0.6;border-radius: 4upx;align-items: center;justify-content: center;padding-bottom: 130upx;">
									<view style="width: 140upx;height: 140upx;border-radius: 100upx; background-color: #000000;align-items: center;justify-content: center;">
	...
	export default {
		data() {
			return {
				...
				Android:'',
				weixin:'',
				schemeUrl:"alipays://platformapi/startapp?appId=60000002"
			}
		},
		async onLoad() {
		...
		methods: {
			...
			getList(type){
				let param={
					"limit": this.limit,
					"page": this.page,
					"pairUserId": uni.getStorageSync('userInfo').userId,
				}
				let opts={ url:'/portal/v1/pair/page/list/map', method:'POST' }
			...
			opreation(item){
				...
				this.pairStatus = item.pairStatus //营业状态
				this.pairId = item.pairId //场所id
			},
			placeDel(){
				this.$refs.showOperation.close()
			...
			remind_confirm(){
				...
				if(this.remind_uniPopup_type == 1){ //改变状态
					let value = 0
					if(this.pairStatus==0){
						value=1
					}
					if(this.pairStatus==1){
						value=0
					}
					let param={
						"pairId":this.pairId,
					...
					let opts={ url:'/portal/v1/pair/update/status', method:'POST' }
					this.http.httpTokenRequest(opts,param).then(
						res => {
							if(res.data.code == 200){
								this.pairStatus = value
								//刷新列表
								for(let i=0;i<this.list.length;i++){
									if( this.list[i].pairId == this.pairId){
										this.list[i].pairStatus = this.pairStatus
									}
								}
							}else{
								uni.showToast({
				...
				if(this.remind_uniPopup_type == 2){ //是否删除
					...
					let opts={ url:'/portal/v1/pair/remove', method:'POST' }
					this.http.httpTokenRequest(opts,param).then(
						res => {
							if(res.data.code == 200){
								//查询订单列表
