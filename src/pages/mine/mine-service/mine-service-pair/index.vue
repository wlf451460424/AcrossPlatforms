					...
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;padding: 8upx 12upx;">
							...
							<view v-for="(item,index) in list" :key="index"  @click="go_Details(item)"  style="flex-direction:column;width: 100%;border-radius: 2px; background-color: #FFFFFF;margin-bottom:4px;padding:6px 6px 0;" >
								...
								<view style="width: 100%;align-items: center;height: 60upx;margin-top: 12upx;color: #b7b7b7;font-size: 24upx;border-top: 1px solid #f9f9f9">
									预约宠物：
									...
									<text >{{item.pairPetBirthday}}</text>
								</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap(e) {
			uni.navigateTo({
				url:"service-pair-placeList?id=" + uni.getStorageSync('userInfo').userId
			})
		},
		async onLoad(e) {
		...
		methods: {
			...
			getorderList(type){
				...
				let opts={ url:'/portal/v1/pair_order/list/status', method:'POST' }
			...
			go_Details(item) {
				uni.navigateTo({
					url:'service-pair-record-details?id=' + item.pairOrderId
				})
			},
