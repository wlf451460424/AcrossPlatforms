<template>
	<view class="content">
		<view class="banner" style="z-index:2;">
		    <image class="banner-img" src="@/static/tag-bg.png"  mode="scaleToFill"></image>
			<view style="position: absolute;width:100%;height: 260upx;background: rgba(0,0,0,0.3);"></view>
		    <view class="banner-title">
			</view>
			<view class="banner-title1">
				<text v-if="introduceData.tagDescription" style="width: 100%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;color: #FFFFFF;" >
					{{introduceData.tagDescription}}
				</text>
			</view>
			<view class="banner-title3" >
		...
		<view v-if="menuView"  style="z-index: 6;position: fixed;margin-top: 344upx;width: 100%;height:100%;background-color:rgba(255,255,255,0);flex-direction: column;">
...
<script>
	...
	export default {
		...
		onShareAppMessage(res) {
		    return {
		      title: this.introduceData.tagDescription,
		...
		methods: {
			...
			getTagInfoById(){
				...
				let str = '/portal/v1/tag/get/' + this.typeId;
					...
					res => {
						if(res.data.code == 200){
							...
							uni.setNavigationBarTitle({
							    title: this.introduceData.tagName
							});
						}
					}
				)
			...
			//获取标签
			getLabel(){
				let param={"classificationTagType":2};//分类标签所属类型 0：小组/1: 话题/2：标签
			...
			getPostList(type){
				...
				if(this.tabCurrentIndex == 0){
					param={
						...
						"page": this.page,
						// "postPetId": ’,//宠物相关的帖子
						// "postTopicId": '',//根据话题查询帖子
						"postTagId": this.typeId,//根据标签查询帖子
						"sortType":0,//全部
					}
					...
					param={
						...
						"postTagId": this.typeId,//根据标签查询帖子
						"sortType":'',
						// "topicMenuType":this.nowSelectTypeId,
			...
			ClickCareBtn(item){
				...
				if(item.ifollowed){
					...
					let param={
						...
						"followTargetType": 20, //关注目标类型 0-文章 1-笔记 2-动态 3-问答 4-新闻 5-评论 6-回复 7-用户 8-话题 9-小组 10-赞 11-收藏 12-分享 13-通知  14-关注  20-标签
					} ;
			...
			look(item){
				...
				uni.navigateTo({
					url:'/pages/group/group-member?id=' + item.tagId +'&type=20'
				})
			},
			//退出小组
			quit(item){
