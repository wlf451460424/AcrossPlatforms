		...
		<view style="z-index: 2;display: flex;flex-direction: row;width: 100%;height: 80upx;">
			<scroll-view id="nav-bar" class="nav-bar" scroll-x scroll-with-animation :scroll-left="scrollLeft" style="overflow: hidden;background-color: #FFFFFF;padding-right: 10upx;">
				<view 
					...
					:id="'tab' + index"
					@click="changeTab(index,item.tagGroupId)"
				>{{item.tagGroupName}}</view>
			</scroll-view>
			...
			<scroll-view scroll-y="true" style="max-height: 600upx;">
				<view style="display: flex;flex-direction: column;padding: 20upx 20upx;background-color: #FFFFFF;">
					<view style="flex-direction: row; width: 100%;height: 100%; flex-wrap: wrap;">
						<view @click="selectItemClick(index,item.tagGroupId)" v-for="(item,index) in tabBars" :key="index" :id="item.tagGroupId"
							style="height:70upx;line-height: 70upx;display: inline-block;padding:0 20upx;margin:10upx;">
								<text>{{item.tagGroupName}}</text>
						</view>
					</view>
				</view>
			</scroll-view>
...
<script>
	...
	export default {
		...
		async onLoad(e) {
			...
			let str = "/portal/v1/channel/get/" + this.channelId
					...
					if(res.data.code == 200 ){
						//设置标题
			 			uni.setNavigationBarTitle({
			 			    title: res.data.data.channelName
			 			});
					}
				}
			)
			...
			//查询标签菜单
			param={"channelId": this.channelId,}
			str = "/portal/v1/channel/get/list/" + this.channelId
			opts={ url:str, method:'POST' }
			this.http.httpTokenRequest(opts,param).then(
				res => {
					if(res.data.code == 200 ){
						...
						if(typeArr.length){
							...
							this.nowSelectTypeId = this.tabBars[0].tagGroupId;
							//获取帖子列表
							this.onPulldownReresh();
						}else{
		...
		methods: {
			...
			getPostList(type){
				...
				if(this.tabCurrentIndex == 0){
					...
					opts={ url:'/portal/v1/channel/page/list', method:'POST' }
					...
					param={
						...
						"sortType": 0,  // 0:全部 1推荐
						"channelId": this.channelId, 
						"tagGroupId":this.nowSelectTypeId
					}
					opts={ url:'/portal/v1/channel/page/list', method:'POST' }
				}
				this.http.httpTokenRequest(opts,param).then(
