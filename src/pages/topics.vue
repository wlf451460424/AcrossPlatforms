				...
				<swiper-item>
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;">
							<!-- 话题 -->
							<view v-for="(item,index) in list" :key="index" style="flex-direction:row;width: 100%;background-color: #FFFFFF;margin-bottom:8upx;padding: 0 10upx;" >
								...
								<view v-if="!item.ifollowed" @click="ClickCareBtn(item)" style="min-width: 20%;align-items:center;justify-content: flex-end;">
			...
			<scroll-view scroll-y="true" style="max-height: 600upx;">
				<view style="display: flex;flex-direction: column;padding: 20upx 20upx;background-color: #FFFFFF;">
					<view style="flex-direction: row; width: 100%;height: 100%; flex-wrap: wrap;">
						<view @click="selectItemClick(index,item.topicTypeId)" v-for="(item,index) in tabBars" :key="index" :id="item.topicTypeId"
							style="height:70upx;line-height: 70upx;display: inline-block;padding:0 20upx;margin:10upx;">
								<text>{{item.topicTypeName}}</text>
						</view>
					</view>
				</view>
			</scroll-view>
