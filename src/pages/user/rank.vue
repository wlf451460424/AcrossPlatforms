<template>
	<view class="content">
		...
		<!-- <scroll-view id="nav-bar-1" class="nav-bar-1" scroll-x scroll-with-animation :scroll-left="scrollLeft" style="overflow: hidden;background-color: #FFFFFF;">
			<view
				v-for="(item,index) in tabBarsList" :key="index"
				class="nav-item-1"
				:class="{current: index === tabCurrentIndex_1}"
				:id="'tab_1' + index"
				@click="changeTab_1(index)"
			>{{item.name}}</view>
		</scroll-view> -->
							...
							<view v-if="list.length" style="z-index: 2;display: flex;flex-direction: row;width: 100%;height: 440upx;background-color: #FFFFFF;padding-bottom: 10upx;">
								<view style="width: 33.3%;height: 100%;padding: 50upx 6upx 0 12upx;">
									<view style="flex-direction: column;width:100%;height: 100%;align-items: center;padding: 12upx;border: 2upx solid #f1f1f1;border-top-right-radius: 20upx;border-top-left-radius: 20upx;">
										...
										<!-- <text style="max-width: 85%;height: 40upx;line-height:40upx;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;">
											{{this.list[1].userNickname}}
										</text> -->
	...
	export default {
		...
		methods: {
			...
			async changeTab_1(e){
				...
				// }
				// if(typeof tabBar !== 'object'){
				// 	tabBar = await this.getElSize("nav-bar-1")
				// }
				// //计算宽度相关
				// let tabBarScrollLeft = tabBar.scrollLeft;
				// let width = 0; 
				...
				// //获取可滑动总宽度
				// for (let i = 0; i <= index; i++) {
				// 	let result = await this.getElSize('tab_1' + i);
				// 	width += result.width;
				// 	if(i === index){
				// 		nowWidth = result.width;
				// 	}
