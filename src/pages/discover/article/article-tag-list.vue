			...
			this.http.httpTokenRequest(opts,param).then(
				res => {
					if(res.data.code == 200){
						...
						if(typeArr.length){
							var tags = typeArr[this.typeIndex].tags
			...
			getPostList(type){
				...
				if(this.tabCurrentIndex == 0){
					param={
						"limit": this.limit,
						"page": this.page,
						"postType": 0,  //类型 0: 文章 1：笔记 2：动态 3：问答 4：新闻
						"postTagId": '', //（该字段不传的时候默认查询全部数据）
						"postTagCategory":this.tagCategoryId
					}
					...
					param={
						...
						"page": this.page,
						"postType": 0,  //类型 0: 文章 1：笔记 2：动态 3：问答 4：新闻
						"postTagId": this.nowSelectTypeId, //（该字段不传的时候默认查询全部数据）
						"postTagCategory":this.tagCategoryId
					}
					opts={ url:'/portal/v1/post/page/list/all', method:'POST' }
				}
