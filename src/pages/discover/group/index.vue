<template>
	<view class="content">
		<view style="width: 100%;background-color: #FFFFFF;margin-bottom: 4upx;padding: 12upx;">
			<text style="font-weight: 700;">热门小组</text>
		</view>
		<view style="flex-direction: row;width: 100%;flex-wrap: wrap;justify-content: center;">
			<view v-for="(item,index) in cardBars" :key="index" @click="click_cardBars(item)"
				...
				<view style="width: 140upx; height: 100%;justify-content:center;align-items:center;">
					<image :src="decodeURIComponent(item.groupAvatar)" style="width: 90upx; height: 90upx; border-radius: 50upx;"></image>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			click_cardBars(item){
				uni.navigateTo({
					url:'group-details?groupId='+item.groupId
				})
			},
			ClickCareBtn(item){
				//添加/修改关注状态
