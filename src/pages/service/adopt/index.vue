					...
					<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore" >
						<view style="flex-direction: column; width: 100%;flex-flow: wrap;padding:6upx 0 12upx 6upx;">
							<view v-for="(item,key) in list" :key="key" @click="gofosterageItem_Details(item)" style="flex-direction: column;width: 48%; height: 530upx;margin:6upx;display:block;background: #FFFFFF;border-radius: 4upx;">
								<view style="flex-direction: column;">
									...
									<view style="flex-direction: column;padding-top: 6upx;">
										<view style="width: 100%;padding:0 12upx;">
											...
											<!-- <text style="width: 100upx;">{{adopt_status[item.fosterAdoptStatus]}}</text> -->
										</view>
								...
								<view v-if="item.fosterAdoptStatus == 1" style="position: relative;top:-520upx;left:0;width: 100%;height: 530upx;background: #FFFFFF;opacity: 0.6;border-radius: 4upx;align-items: center;justify-content: center;padding-bottom: 130upx;">
	...
	export default {
		data() {
			return {
				...
				sortDirection:0,	//排序方向 0：正序/1：倒序,
			}
		},
		async onLoad() {
		...
		methods: {
			...
			getPostList(type){
				...
				let opts={ url:'/portal/v1/foster/page/list', method:'POST' }
						...
						if(res.data.code == 200 ){
							if(res.data.data.records.length ==0){//没有数据
								...
								for(let i=0;i<arr.length;i++){
									...
									arr[i].fosterAdoptPoi = arr[i].fosterAdoptPoi.split("|")[1]
