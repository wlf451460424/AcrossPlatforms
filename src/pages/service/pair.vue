<template>
	<view class="content">
		<view v-if="reciveData.isDeleted == 0" style="flex-direction: column;width: 100%; ">
			<swiper @click="gotoImageShow(reciveData.pairPetImage)" class="swiper-style" indicator-dots="true" :autoplay="true" :interval="2000" :duration="500">
			  <swiper-item v-for="(item,index) in reciveData.pairPetImage" :key="index">
			...
			<view style="width: 100%;flex-direction: column;padding: 0 20upx;background-color: #FFFFFF;">
				<text style="font-size:40upx;font-weight: 700;line-height: 100upx;">{{reciveData.pairTitle}}</text>
				<view style="width: 100%;">
					<view style="width: 80%;align-items: center;">
						<text style="width:75%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;">
							{{reciveData.score}}
						</text>
					</view>
			...
			<view style="width: 100%;flex-direction: column;padding: 0 20upx;background-color: #FFFFFF;margin-top: 10upx;">
				<view @click="gotoPetDetailsPage(reciveData)" style="width: 100%;height: 80upx;margin: 20upx 0;">
					<view style="width: 80%;align-items: center;">
						<image :src="decodeURIComponent(reciveData.petAvatar)" style="width:80upx;height:80upx;border-radius: 40upx;margin-right: 20upx;"></image>
						<text style="width:75%;display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1; overflow: hidden;">
							{{reciveData.petNickname}}
						</text>
					</view>
					<view style="width: 20%;justify-content: flex-end;align-items: center;">
			...
			<view style="width: 100%;padding:20upx 30upx;background-color: #FFFFFF;">
				...
				<view style="width: 25%;flex-direction: column;align-items: center;">
					...
					<view style="height: 80upx;align-items: center;">
						<text>{{greden[reciveData.pairPetGender]}}</text>
					</view>
				</view>
				<view style="width: 25%;flex-direction: column;align-items: center;">
					<image src="@/static/service/brithday.png" style="width: 60upx;height: 60upx;"></image>
					<text style="color: #555;line-height: 80upx">{{reciveData.pairPetBirthday}}</text>
				</view>
				<view style="width: 25%;flex-direction: column;align-items: center;">
					<image src="@/static/service/weight.png" style="width: 60upx;height: 60upx;"></image>
					<text style="color: #555;line-height: 80upx">{{reciveData.pairPetWeight}}</text>
				</view>
			</view>
			
			<view style="width: 100%;flex-direction: column;padding:0 20upx;color: #888888;background-color: #FFFFFF;margin-top: 2upx;">
				<text style="line-height: 70upx;font-weight: bold;color: #555;" >宠物信息</text>
				<view style="width: 100%; padding: 10upx 20upx;">
					<view style="width:50%;align-items: center;">
						<image :src="optionIco[reciveData.pairPetInternalDewormingStatus]" style="width: 30upx;height: 30upx;margin-right: 10upx;"></image>
						<text>{{dewormingStatus_1[reciveData.pairPetInternalDewormingStatus]}}</text>
					</view>
					<view style="width:50%;align-items: center;">
						<image :src="optionIco[reciveData.pairPetExternalDewormingStatus]" style="width: 30upx;height: 30upx;margin-right: 10upx;"></image>
						<text>{{dewormingStatus_2[reciveData.pairPetExternalDewormingStatus]}}</text>
					</view>
				</view>
				<view style="width: 100%; padding: 10upx 20upx;">
					<view style="width:50%;align-items: center;">
						<image :src="optionIco[reciveData.pairPetImmunityStatus]" style="width: 30upx;height: 30upx;margin-right: 10upx;"></image>
						<text>{{immunityStatus[reciveData.pairPetImmunityStatus]}}</text>
					</view>
					<view style="width:50%;align-items: center;">
						<image :src="optionIco[1]" style="width: 30upx;height: 30upx;margin-right: 10upx;"></image>
						<text>血统：{{bloodline[reciveData.pairPetBloodline]}}</text>
					</view>
				</view>
				<view style="width: 100%; padding: 10upx 20upx;">
					<view style="width:50%;align-items: center;">
						<image :src="optionIco[1]" style="width: 30upx;height: 30upx;margin-right: 10upx;"></image>
						<text>{{reciveData.pairPaid==1?'有偿':'无偿'}}</text>
					</view>
				</view>
			</view>
			
			<view style="width: 100%;flex-direction: column;padding: 0 20upx 20upx;background-color: #FFFFFF;margin-top: 4upx;">
				<text style="line-height: 70upx;font-weight: bold;">配对说明</text>
				<rich-text class="richText" :nodes="reciveData.pairDescription" style="color: #888888;"></rich-text>
			</view>
			<view style="width: 100%;align-items: center;color: #888888;background-color: #FFFFFF;padding:12upx 20upx;margin-top: 4upx;">
				<image src="@/static/actionImg/Location_ico.png" style="width: 30upx;height: 36upx;margin-right: 10upx;"></image>
				<text style="width:95%;line-height: 50upx;display: -webkit-box; -webkit-box-orient: vertical; overflow: hidden;">{{reciveData.pairAddress}}</text>
			</view>
			
			<view style="flex-direction: row;width: 100%; height: 80upx; line-height: 100upx;margin-top: 4upx;
			...
			<view style="flex-direction: column;width: 100%;">
				<view v-for="(item,index) in commentList" :key="index" style="flex-direction: column;width: 100%;padding: 20upx 20upx;background-color: #FFFFFF;margin-bottom: 2upx;">
					...
					<text @click="gotoHuifu_page(item)" style="display: -webkit-box; -webkit-box-orient: vertical; overflow: hidden;padding-top: 6upx;">
			...
			<view v-if="gotoBtnShow && reciveData.pairStatus==0 && reciveData.pairAuditResult==2" style="width: 100%;justify-content: center;color: #FFFFFF;padding:20upx;background-color: #FFFFFF;">
				<view @click="gotoAppointment" :class="{styleNone:reciveData.pairStatus}" style="width: 100%;height: 70upx;border-radius: 10upx;background-color: #f76d45;justify-content: center;align-items: center;">
					<text>{{reciveData.pairStatus==0?'去预约':'暂停营业'}}</text>
				</view>
			</view>
	...
	export default{
		...
		data() {
			...
			return{
				...
				greden:["弟弟","妹妹","未知"],
				bloodline:["纯血级","伴侣级","赛级","纯种级","冠军级","其它"],
		...
		onShareAppMessage(res) {
		    return {
		      title: this.reciveData.pairTitle,
		...
		onShareTimeline() {
			return {
				...
				imageUrl: this.reciveData.pairPetImage[0]
			}
		},
		// #endif
		...
		methods: {
					...
					res => {
						if(res.data.code == 200){
							...
							for(var i=0;i<arr.length;i++){
								if(arr[i].copywritingConfigurationType == 11){//寄养须知
			...
			//获取详情
			getDetails(value){
				let param={ "pairId":value }
				let str = '/portal/v1/pair/get/' + value
				let opts={ url:str, method:'POST' }
				this.http.httpTokenRequest(opts,param).then(
					res => {
						if(res.data.code == 200 || res.data.data){
							this.reciveData = res.data.data
							this.reciveData.pairPetImage = res.data.data.pairPetImage.split(",")
							this.reciveData.pairPetBirthday = this.getBirthday(res.data.data.pairPetBirthday)
							this.ContactPhoneNumber = res.data.data.pairContact
							this.reciveData.userExperienceYears = this.getDaysBetween(res.data.data.userPetTime)
							if(this.reciveData.pairPetWeight > 0){
								this.reciveData.pairPetWeight = this.reciveData.pairPetWeight + 'kg'
							}else{
								this.reciveData.pairPetWeight = '未知'
							}
							
							//去预约 去申请
							if(this.reciveData.pairUserId == uni.getStorageSync('userInfo').userId){
			...
			getCommentList(value){
				let param={ 
					...
					"evaluationType": 1,  //服务类型 0:寄养 1:配对
				}
					...
					res => {
						if(res.data.code == 200 || res.data.data){
							...
							for(var i=0;i<this.commentList.length;i++){
								...
								if(this.commentList[i].evaluationUserId == this.reciveData.pairUserId){
			...
			placeReport(){
				this.$refs.showOperation.close()
				uni.navigateTo({   //  11. 配对详情
					url:'/pages/discover/report/index?id='+ this.reciveData.pairId + '&type=11' 
				})
			},
			placeDel(){
				this.$refs.showOperation.close()
				let param={
					"pairId":this.reciveData.pairId,
				} ;
			...
			//举报
			jubao(){
				uni.navigateTo({   //   11. 配对详情 12. 配对详情评价 13. 配对详情回复
					url:'/pages/discover/report/index?id='+this.jubao_Info.evaluationId + '&type=12' 
				})
			...
			gotoPetDetailsPage(item){
				uni.navigateTo({
					url:'/pages/mine/pet/index?id=' + item.pairPetId + '&name=' + item.petNickname
				})
			},
			...
			gotoHuifu_page(item){
				// uni.navigateTo({
				// 	url:'service/foster/foster-place-reply-list?id=' + item.evaluationId  + '&type=1'
				// })
			},
			//跳转评论页面
			gotoPinlun_page(){
				// #ifdef H5
				//以前的唤醒方式
				//this.calculation.wakeUp_app('toServeMoreComment?serveSchemeId='+ this.id + '&serveSchemeType=1')
				//友盟的唤醒方式
				this.calculation.um_wakeUp_app("serviceDetails",{serveSchemeId:this.id,serveSchemeType:1})
				//  #endif
				// #ifdef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU 
				uni.navigateTo({
					url:'/pages/service/foster/foster-place-comment-list?id=' + this.id   + '&type=1'
				})
				// #endif
			},
			gotoAppointment(){
				// #ifdef H5
				//以前的唤醒方式
				// this.calculation.wakeUp_app('toServePromise?serveSchemeId='+ this.id + '&serveSchemeType=1')
				//友盟的唤醒方式
				this.calculation.um_wakeUp_app("serviceDetails",{serveSchemeId:this.id,serveSchemeType:1})
				//  #endif
				// #ifdef MP-WEIXIN || MP-TOUTIAO || MP-QQ || MP-BAIDU 
